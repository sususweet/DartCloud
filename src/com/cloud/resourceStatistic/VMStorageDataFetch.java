...
public class VMStorageDataFetch 
{
	public ArrayList<VMStorage> fetchVMStorage(String ip)
	{
		ArrayList<VMStorage> lis = new ArrayList<VMStorage>();
		...
		String vmStorageStartTime = null;//记录CPU的开始监控时间
		String vmStorageEndTime = null;//记录CPU的结束监控时间
		...
		try {
			...
			//***************获取开始监控时间************************************************
			String fecthOrder = "rrdtool first " + vmDirectory + "/disk_free.rrd";//获取监控CPU的开始时间
			sess.execCommand(fecthOrder);
			InputStream stdout = new StreamGobbler(sess.getStdout()); 
			BufferedReader br = new BufferedReader(new InputStreamReader(stdout));
			vmStorageStartTime = br.readLine();
			sess.close();
			...
			fecthOrder = "rrdtool last " + vmDirectory + "/disk_free.rrd";//获取监控CPU的结束时间
			sess.execCommand(fecthOrder);
			stdout = new StreamGobbler(sess.getStdout()); 
			br = new BufferedReader(new InputStreamReader(stdout));
			vmStorageEndTime = br.readLine();
			sess.close();
			...
			sess = conn.openSession();
			fecthOrder = "rrdtool fetch " + vmDirectory + "/disk_free.rrd AVERAGE -s " + vmStorageStartTime + " -e " + vmStorageEndTime;
			sess.execCommand(fecthOrder);
			stdout = new StreamGobbler(sess.getStdout()); 
			br = new BufferedReader(new InputStreamReader(stdout));
			br.readLine();
			...
			sess = conn.openSession();
			fecthOrder = "rrdtool fetch " + vmDirectory + "/disk_total.rrd AVERAGE -s " + vmStorageStartTime + " -e " + vmStorageEndTime;
			sess.execCommand(fecthOrder);
			stdout = new StreamGobbler(sess.getStdout()); 
			br = new BufferedReader(new InputStreamReader(stdout));
			br.readLine();
			...
			{ 
				...
				{
					int t = Integer.valueOf(a[1].substring(15, 16));//取出科学计数法中的幂，即字符串最后一位数字
					lis.get(i).storage_Total = Double.valueOf(a[1].substring(0, 7)) * Math.pow(10, t);//保留两位有效数字
				}
				i ++;
			}
			sess.close();
